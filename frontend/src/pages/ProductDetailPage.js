...
import { ShoppingCart, Minus, Plus, Star, Package, Truck, CheckCircle } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        setRelatedProducts(relatedRes.products.filter(p => p.product_id !== id));
      } catch (error) {
        ...
        <div className="grid md:grid-cols-2 gap-12 mb-16">
          ...
          <div>
            ...
            <div className="flex space-x-2">
              {[1, 2, 3].map(i => (
                <div key={i} className="w-20 h-20 rounded-lg overflow-hidden border-2 border-gray-200 cursor-pointer hover:border-amber-600">
          ...
          <CardContent className="p-6">
            <Tabs defaultValue="description">
              ...
              <TabsContent value="description" className="text-gray-700">
                ...
                <p>This authentic product brings the rich flavors of {product.country} directly to your kitchen. Perfect for traditional recipes and modern fusion dishes alike.</p>
              </TabsContent>
              <TabsContent value="ingredients" className="text-gray-700">
                <h3 className="font-semibold text-lg mb-3">Ingredients</h3>
                <p>100% natural ingredients sourced from {product.region}. No artificial preservatives or colors.</p>
              </TabsContent>
              <TabsContent value="storage" className="text-gray-700">
                <h3 className="font-semibold text-lg mb-3">Storage Instructions</h3>
                <p>Store in a cool, dry place away from direct sunlight. Refrigerate after opening for maximum freshness.</p>
              </TabsContent>
                ...
                <div className="space-y-4">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="border-b pb-4">
                      ...
                      <p className="text-sm">Excellent product! Authentic taste and great quality.</p>
                    </div>
                  ))}
                </div>
              </TabsContent>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map(relatedProduct => (
                <Link key={relatedProduct.product_id} to={`/product/${relatedProduct.product_id}`}>
                  <Card className="hover:shadow-xl transition-all duration-300">
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="text-xl font-bold text-amber-600">${relatedProduct.price}</div>
                    </CardContent>
                  </Card>
                </Link>
              ))}
