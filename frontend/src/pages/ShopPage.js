...
export const ShopPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const culture = location.pathname.includes("african")
          ? "African"
          : location.pathname.includes("latino")
          ? "Latino"
          : "";
  ...
  const handleAddToCart = (product) => {
    ...
    toast.success(
      `${product.name} ${t("cart.itemRemoved").replace(
        "removed from",
        "added to"
      )}`
    );
  ...
  const getPageTitle = () => {
    ...
    if (selectedFilters.category)
      return selectedFilters.category.replace(/-/g, " ");
    if (selectedFilters.region)
      return selectedFilters.region.replace(/-/g, " ");
    if (selectedFilters.search)
      return `Search results for "${selectedFilters.search}"`;
    return "All Products";
  };
