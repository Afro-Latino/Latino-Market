...
async def register(user_data: UserCreate):
    ...
    return {"user": UserResponse(**user.dict()), "session_token": token}
...
async def create_product(
    ...
    await db.categories.update_one(
        {"name": product.category}, {"$inc": {"product_count": 1}}
    )
...
async def update_product(
    ...
    if "category" in update_data and update_data["category"] != existing["category"]:
        await db.categories.update_one(
            {"name": existing["category"]}, {"$inc": {"product_count": -1}}
        )
        await db.categories.update_one(
            {"name": update_data["category"]}, {"$inc": {"product_count": 1}}
        )
...
async def get_recipes(culture: Optional[str] = None, search: Optional[str] = None):
...
async def get_orders(
    ...
    """Get user's orders"""
    user = await get_current_user(db, authorization, session_token)
    orders = await db.orders.find({"user_id": user.user_id}, {"_id": 0}).to_list(
        length=100
    )
...
async def get_stripe_credentials():
    ...
    settings = await db.site_settings.find_one(
        {"settings_id": "site_settings"}, {"_id": 0}
    )
...
async def stripe_checkout(order_id: str, request: Request):
    ...
    stripe_checkout = StripeCheckout(
        api_key=stripe_api_key, webhook_url=f"{host_url}/api/payments/stripe/webhook"
    )
    ...
    checkout_request = CheckoutSessionRequest(
        ...
        metadata={"order_id": order_id, "user_id": order.get("user_id", "guest")},
    )
...
async def stripe_webhook(request: Request):
    ...
    try:
        ...
        if event.event_type == "checkout.session.completed":
            ...
            # Update order
            transaction = await db.payment_transactions.find_one(
                {"stripe_session_id": event.session_id}, {"_id": 0}
            )
            if transaction:
                await db.orders.update_one(
                    {"order_id": transaction["order_id"]},
...
async def update_settings(
    ...
    await db.site_settings.update_one(
        {"settings_id": "site_settings"}, {"$set": settings_update}, upsert=True
    )
...
async def get_notices():
    ...
    notices = await db.holiday_notices.find(
        {"is_active": True, "start_date": {"$lte": now}, "end_date": {"$gte": now}},
        {"_id": 0},
    ).to_list(length=10)
...
async def get_all_notices(
    ...
    await get_current_admin(db, authorization, session_token)
    notices = (
        await db.holiday_notices.find({}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(length=100)
    )
    return {"notices": notices}
...
async def update_notice(
    ...
    await db.holiday_notices.update_one(
        {"notice_id": notice_id}, {"$set": notice_update}
    )
...
async def update_blog_post(
    ...
    await db.blog_posts.update_one({"post_id": post_id}, {"$set": post_update})
...
async def get_announcements():
    ...
    announcements = (
        await db.announcements.find({"is_active": True}, {"_id": 0})
        .sort("priority", -1)
        .limit(5)
        .to_list(length=5)
    )
...
async def get_all_announcements(
    ...
    await get_current_admin(db, authorization, session_token)
    announcements = (
        await db.announcements.find({}, {"_id": 0})
        .sort("created_at", -1)
        .to_list(length=100)
    )
    return {"announcements": announcements}
...
async def update_announcement(
    ...
    await db.announcements.update_one(
        {"announcement_id": announcement_id}, {"$set": announcement_update}
    )

    updated = await db.announcements.find_one(
